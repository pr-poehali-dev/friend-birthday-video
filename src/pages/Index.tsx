...
const Index = () => {
  ...
  const handleMainVideoUpload = (file: File) => {
    ...
    setUploadedMainVideo(videoUrl);
    setShowMainUpload(false);
  };

  const handleBlooperVideoUpload = (file: File) => {
    const videoUrl = URL.createObjectURL(file);
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-yellow-50 to-purple-50 relative overflow-hidden">
      ...
      <div className="text-center py-12 relative z-10">
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl max-w-2xl mx-auto p-8 animate-fade-in-up" style={{ animationDelay: '1s' }}>
          ...
          <p className="text-lg text-gray-700">Анюта, ты — человек, для которого мы готовы устроить спецвыпуск новостей, составить четыре графика, опросить плюшевых зверей и снять кучу неудачных, но смешных дублей 🎬😂 Потому что ты этого стоишь ✨ Ты — наше «вау», даже когда всё вокруг «ну такое» 🌥 Ты — наша радость 🥰 Наш стабильно хороший день ☀️ С тобой легче дышать, проще быть собой и всегда есть кому скинуть забавный мем в 3 ночи😹 Мы тебя очень любим❤️ Ты умеешь быть тёплой, смешной, настоящей, и мы каждый день благодарим судьбу за то, что ты с нами ✨ С днём рождения, солнышко 🎂 Ты — подарок, который мы получаем каждый день 🎁💛 Обнимаем, целуем и передаём всю свою любовь: Аня, Наташа и Андрей 💛</p>
        </div>
      </div>
